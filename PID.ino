...
long serialTime = 1000;
volatile int runs = 0;
double left_speed_val = 0, right_speed_val = 0, left_encoder_val = 0, right_encoder_val = 0;
double kp = .1,ki = 0,kd = 0;   //Values for the PID
double old_left_encoder_val = 0, old_right_encoder_val = 0;
...
//  if(aTune.Runtime() == 1){ //atune has completed
//      kp = aTune.GetKp();
//      ki = aTune.GetKi();
//      kd = aTune.GetKd();
//      leftPID.SetTunings(kp,ki,kd);
//      Serial.println(leftPID.Compute());
//      moveForward(right_speed_val, left_speed_val, 1000); // right left distance
//  }
...
//  Serial.print(" "); 
  ...
  if(millis()>serialTime)
  {
    serialTime+=500;
    Serial.print("left: \t");
    Serial.println(left_encoder_val - old_left_encoder_val);
    old_left_encoder_val = left_encoder_val;
    Serial.print("right: \t");
    Serial.println(right_encoder_val - old_right_encoder_val);
    old_right_encoder_val = right_encoder_val;
  }
